...
class AllogModel  extends Model {
    ...
    public function get_lista_ventas($fecha1,$fecha2,$mov,$numero,$server) {
        $sql = "SELECT CONVERT(VARCHAR(26),ALL_FECHA_PRO,23) ALL_FECHA_PRO,T1.ALL_NUMOPER,RTRIM(T1.ALL_NUMSER) ALL_NUMSER,ALL_NUMFAC,T2.FAR_NUMFAC,RTRIM(ALL_CONCEPTO) ALL_CONCEPTO,ALL_IMPORTE_AMORT,rtrim(ALL_CTAG1) ALL_CTAG1 ";
         $sql.= 'FROM ALLOG T1 ';
         $sql.= "LEFT JOIN FACART T2 ON(T2.FAR_CODCIA=T1.ALL_CODCIA AND T2.FAR_FBG=T1.ALL_FBG AND T2.FAR_NUMFAC=T1.ALL_NUMFAC AND T2.FAR_NUMSER=T1.ALL_NUMSER AND T2.FAR_TIPMOV=T1.ALL_TIPMOV AND T2.FAR_CODART IN (SELECT ART_KEY FROM ARTI AS T3 INNER JOIN TABLAS AS T4 ON(T3.ART_CODCIA = T4.TAB_CODCIA AND T3.ART_SUBGRU = T4.TAB_NUMTAB AND TAB_CONTABLE2 LIKE 'C%'))) ";
         $sql.= 'WHERE ALL_CODCIA in (\'25\') AND  ';
         $sql.= 'ALL_TIPMOV = '.$mov.' AND ';
         $sql.= 'ALL_FECHA_PRO >= \''.$fecha1.'\'  AND ';
         $sql.= 'ALL_FECHA_PRO <= \''.$fecha2.'\'  AND ';

...
GO
-- Stored Procedure para generar comprobante de consultorio - Versión 3
-- Evita pagos duplicados marcando servicios como pagados
CREATE PROCEDURE [dbo].[SP_GenerarComprobanteConsultorio_v3]
    @CitaId INT,
    @LocalPago VARCHAR(2),
...
BEGIN
    ...
    BEGIN TRY
        ...
        IF @CodClie IS NULL
        BEGIN
            SET @Resultado = 'ERROR: Cita no encontrada - ID: ' + CAST(@CitaId AS VARCHAR(10))
            RETURN
        END
        
        -- Verificar que hay servicios pendientes
        SELECT @ServiciosPendientes = COUNT(*), @Subtotal = SUM(CNS_CANTIDAD * CNS_PRECIO)
        FROM CITAS_SERVICIOS 
        WHERE CNS_CODCIT = @CitaId AND ISNULL(CNS_PAGADO, 0) = 0
        
        IF @ServiciosPendientes = 0 OR @Subtotal IS NULL OR @Subtotal = 0
        BEGIN
            SET @Resultado = 'ERROR: No hay servicios pendientes para generar comprobante - Cita: ' + CAST(@CitaId AS VARCHAR(10))
            RETURN
        END
        ...
        -- Insertar en ALLOG (usando @FechaHoy para fechas)
        INSERT INTO ALLOG (
        ...
        BEGIN
            INSERT INTO FACART (
                ...
                0, 'OPER03    ', 'S', ' ', 0.0488, ' ', 
                @HoraActual, 0, @Cantidad, 1, 'FA', 'N', -- AQUÍ USAMOS @HoraActual
        ...
        -- Verificar que se insertaron los servicios
        IF @InsertedRows = 0
        BEGIN
            SET @Resultado = 'ERROR: No se pudieron insertar servicios en FACART'
            ROLLBACK TRANSACTION
            RETURN
        END
        ...
        -- Insertar en CITAS_PAGOS (usando GETDATE() completo para CTP_FECHA)
        INSERT INTO CITAS_PAGOS (
            ...
            @CitaId, GETDATE(), @Total, @FormaPago, @Referencia, -- GETDATE() completo aquí
            @LocalPago, 1, 1, @TipoComprobante, 
            @NumSer, @NumFac, @CodCia
        )
        
        -- Verificar inserción en CITAS_PAGOS
        IF @@ROWCOUNT = 0
        BEGIN
            SET @Resultado = 'ERROR: No se pudo insertar en CITAS_PAGOS'
            ROLLBACK TRANSACTION
            RETURN
        END
    ...
    BEGIN CATCH
        ...
        SET @Resultado = 'ERROR: ' + ERROR_MESSAGE() + ' (Line: ' + CAST(ERROR_LINE() AS VARCHAR(10)) + ')'
    END CATCH
END
GO
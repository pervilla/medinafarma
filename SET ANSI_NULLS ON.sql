...
GO
-- Corrección para manejar fecha y hora por separado
CREATE PROCEDURE [dbo].[SP_GenerarComprobanteConsultorio]
    @CitaId INT,
    @LocalPago VARCHAR(2),
...
BEGIN
    ...
    BEGIN TRY
        ...
        SELECT @ServiciosCount = COUNT(*), @Subtotal = SUM(CNS_CANTIDAD * CNS_PRECIO)
        FROM CITAS_SERVICIOS 
        WHERE CNS_CODCIT = @CitaId
        
        IF @ServiciosCount = 0 OR @Subtotal IS NULL OR @Subtotal = 0
        BEGIN
            SET @Resultado = 'ERROR: No hay servicios para generar comprobante - Cita: ' + CAST(@CitaId AS VARCHAR(10))
            RETURN
        END
